...
if st.button("Filter"):
    ...
    if fasta_file and dta_filter_files:
        ...
        # Parse DTASelect-filter and get a set of all protein loci
        protein_loci = set()
        for dta_filter_file in dta_filter_files:
            ...
            for dta_filter_result in dta_filter_results:
                protein_loci.update({protein_line.locus for protein_line in dta_filter_result.proteins})

        # Remove loci matching these decoy and contaminant flag.
        if decoy_flag:  # *Note python interprets an empty string as falsy
            protein_loci = [locus for locus in protein_loci if decoy_flag not in locus]
        if contaminant_flag:  # *Note python interprets an empty string as falsy
            protein_loci = [locus for locus in protein_loci if contaminant_flag not in locus]

        dta_filter_locus_to_sequence_map = {locus: locus_to_sequence_map[locus] for locus in protein_loci}
